...
module.exports = {
    ...
    postArticleAddHandler(req, res) {
        // 没有登录或者身份信息已经失效
        if(!req.seesion.isLogin) return res.status(401).send({ status: 401, msg: '身份信息已过期!请登陆后重试!' })
        // req.body 是引用数据类型  对象
        // 将req.body的地址值赋值给articleInfo变量
        // req.body和articleInfo同时指向同一个对象
        // 往articleInfo上添加ctime属性等于给req.body上添   加ctime属性
        ...
        conn.query('insert into articles set ?', articleInfo, (err, result) => {
            if(err || result.affectedRows != 1) return res.atatus(500).send({ status: 500, msg: '文章发表失败!请重试!' + err.message })
    ...
    getArticleEditHandler(req, res) {
        // 如果只做登录校验是不严谨的
        // 应该加上作者的校验
        if(!req.seesion.isLogin) return res.redirect('/')
        // 文章ID
        const articleId = parseInt(req.params.id)
